...
function fromAddress(): string {
  ...
  return process.env.EMAIL_FROM || "TheraSim <onboarding@resend.dev>";
}
...
export async function sendMagicLink(email: string, link: string): Promise<void> {
  ...
  <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;max-width:480px;margin:0 auto;color:#1a1d23">
    <h2 style="color:#4f46e5">TheraSim</h2>
    <p>Voici votre lien de connexion. Il est valable 15 minutes et à usage unique.</p>
    <p style="margin:24px 0">
      <a href="${link}" style="background:#4f46e5;color:#fff;text-decoration:none;padding:12px 20px;border-radius:8px;display:inline-block;font-weight:600">
        Se connecter
      </a>
    </p>
  ...
  await send(email, "Votre lien de connexion TheraSim", html);
}
...
export async function sendInvitation(
  ...
  const html = `
  <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;max-width:480px;margin:0 auto;color:#1a1d23">
    <h2 style="color:#4f46e5">${escapeHtml(brandName)}</h2>
