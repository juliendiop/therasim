...
import { creditSchoolCommissionAction, type CreditResult } from "./actions";

export default function SchoolCommissionForm() {
  const [state, action, pending] = useActionState<CreditResult | null, FormData>(
    creditSchoolCommissionAction,
    null,
  );
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <input
            name="note"
            placeholder="ex. Deal École X, contrat annuel"
            className="mt-1 w-full rounded-lg border border-[var(--border)] p-2 text-sm"
          />
        </div>
        <div className="flex items-end">
          <button
            disabled={pending}
            className="w-full rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-medium text-white hover:bg-[var(--accent-hover)] disabled:opacity-50 sm:w-auto"
          >
            {pending ? "…" : "Créditer"}
          </button>
        </div>
